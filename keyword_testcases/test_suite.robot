...
TC_08 Adding a quicknote
    [Documentation]    Verifies that a user can add a quick note to a booking, including images, text, and PDF, and that the note is saved with all details.
    Select Menu In Navigation Wheel    ${DIARY_MODULE}
    Select Timeslot    ${BOOKING_INFO}[input.time]
    Open Booking Details For New Patient    ${BOOKING_INFO}[input.time]
    Click Add Quick Note Button
    Select Quick Note Type    ${QUICKNOTE_INFO}[input.note_type]
    Enter Quick Note Text    ${QUICKNOTE_INFO}[input.text]
    Attach Image To Quick Note    ${QUICKNOTE_INFO}[attach.image_path]
    Attach PDF To Quick Note    ${QUICKNOTE_INFO}[attach.pdf_path]
    Set Quick Note Created By    ${QUICKNOTE_INFO}[input.created_by]
    Set Quick Note Created On    ${QUICKNOTE_INFO}[input.created_on]
    Set Quick Note Visibility    ${QUICKNOTE_INFO}[input.visibility]
    Add Tags To Quick Note    ${QUICKNOTE_INFO}[input.tags]
    Save Quick Note
    Validate Quick Note Is Added With Details    ${EXPECTED_QUICKNOTE_DATA}
