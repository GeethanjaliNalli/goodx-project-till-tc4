...
TC00005_Diary_Add Quicknote - Adding a quicknote
    [Documentation]    Adding a quick note to a booking. Preconditions: User needs to have a booking on a diary made with a new patient (not added to the debtor list yet). Expected Result: Quick Note is created with image, text, and PDF added and displayed correctly.
    ...
    # Step 3-4: Select Quick Note form in the Clinical Sidebar
    Select Quick Note Form    ${CLINICAL_SIDEBAR_QUICKNOTE_FORM}
    # Step 5-9: Add image to the quick note
    Add Image To Quick Note    ${QUICK_NOTE_IMAGE_UPLOAD_BUTTON}    ${QUICK_NOTE_DATA.file.image_path}
    # Step 10: Add text to the quick note
    Add Text To Quick Note    ${QUICK_NOTE_TEXT_FIELD}    ${QUICK_NOTE_DATA.input.quick_note_text}
    # Step 11-12: Upload PDF to the quick note
    Upload PDF To Quick Note    ${QUICK_NOTE_PDF_UPLOAD_BUTTON}    ${QUICK_NOTE_DATA.file.pdf_path}
    # Step 13: Print the quick note
    Print Quick Note    ${QUICK_NOTE_PRINT_BUTTON}
    # Step 14: Validate Quick Note is created with image, text, and PDF displayed correctly
    Validate Quick Note Created With All Attachments    ${QUICK_NOTE_VALIDATION_SECTION}    ${QUICK_NOTE_DATA.input.quick_note_text}    ${QUICK_NOTE_DATA.file.image_path}    ${QUICK_NOTE_DATA.file.pdf_path}
