...
TC_08 Validate User Is Able To Add Quicknote To Booking
    [Documentation]    Verifies that the user can add a quick note (with text, image, and PDF) to a booking and that the quick note is auto-completed and rendered correctly.
    Select Menu In Navigation Wheel    ${DIARY_MODULE}
    Select Timeslot    ${BOOKING_INFO}[input.time]
    Open Booking Details    ${BOOKING_INFO}[input.time]
    Add Quicknote To Booking    ${QUICKNOTE_DATA}
    Validate Quicknote Is Auto Completed And Printout Rendered    ${QUICKNOTE_DATA}
