...
TC00005_Diary_Add Quicknote - Adding a quicknote
    [Documentation]    Adding a quick note to a booking. Preconditions: User needs to have a booking on a diary made with a new patient (not added to the debtor list yet).
    # Step 1-2: Open Diary and open Clinical Sidebar
    Open Diary And Open Clinical Sidebar    ${DIARY_MODULE}    ${select.clinicalSidebar}
    # Step 3-4: Select Quick Note form
    Select Quick Note Form    ${select.formsLibrary}    ${select.quickNoteForm}
    # Step 5-9: Add image to Quick Note
    Add Image To Quick Note    ${quicknote.addImageButton}    ${quicknote.imageUploadInput}    ${quicknote.imageThumbnail}
    # Step 10: Add text to Quick Note
    Add Text To Quick Note    ${quicknote.textInput}    ${QUICK_NOTE_DATA.input.quick_note_text}
    # Step 11-12: Upload PDF to Quick Note
    Upload PDF To Quick Note    ${quicknote.addPdfButton}    ${quicknote.pdfUploadInput}    ${quicknote.pdfFileRow}
